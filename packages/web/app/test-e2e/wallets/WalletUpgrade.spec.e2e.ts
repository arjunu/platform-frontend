...
describe("Upgrade icbm wallet", () => {
  it("do euro upgrade", () => {
    loginFixtureAccount("INV_EUR_ICBM_HAS_KYC_2", {
      ...
      cy.get(tid("wallet.icbm-euro.upgrade-button")).awaitedClick();
      cy.get(tid("modals.tx-sender.upgrade-flow.summery.upgradeSummary.accept")).awaitedClick();
      confirmAccessModal();
  ...
  it("do eth upgrade", () => {
    loginFixtureAccount("INV_ETH_ICBM_NO_KYC_2", {
      ...
      cy.get(tid("wallet.icbm-eth.upgrade-button")).awaitedClick();
      cy.get(tid("modals.tx-sender.upgrade-flow.summery.upgradeSummary.accept")).awaitedClick();
      confirmAccessModal();
      cy.get(tid("modals.shared.signing-message.modal"));
      cy.get(tid("modals.shared.tx-success.modal"));
